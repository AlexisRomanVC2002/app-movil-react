...
const VisFoto = ({nc, setIsFotoSelected, urlPhoto}) => {

 const [image, setImage]=useState(urlPhoto == null ? "" : urlPhoto);
 const [isLoading, setIsLoading]= useState(false);

 useEffect(() => {
  setImage(urlPhoto == null ? "" : urlPhoto);
 },  [urlPhoto]);
 ...
 const uploadImageAsync=async(uri)=>{
  ...
  return (
      <SafeAreaView style={{flex:1, alignItems:'center',justifyContent:'center' }}> 
        <View>
              <>
                {/*"Para seleccionar la imagen"*/}
                  <TouchableOpacity style={{alignItems: 'center', backgroundColor: '#fff',  padding: 10}}
                    onPress={pickImage}  
                  >
                    {isLoading ? (
        ...
        {!image ? (
            <View>
               <Text style={{color: "white"}}>No se ha seleccionado ningúna imagen</Text>
             </View>
          ) : (
          <>
              {image && (
